...
INSERT INTO friendbook_author
VALUES('SHALINI',2,'KABUL','AFGHANISTAN',25,'FEMALE')
INSERT INTO friendbook_author
...
INSERT INTO friendbook_author
VALUES('ANEESHA',8,'DURGAPUR','INDIA',47,'FEMALE')
INSERT INTO friendbook_author
...
INSERT INTO friendbook_sizepost
VALUES('WORK HARD',70,'STATUS',7)
INSERT INTO friendbook_sizepost
VALUES('CHANGED MYSELF',80,'STATUS',8)
INSERT INTO friendbook_sizepost
VALUES('WATCH IT',90,'VIDEO',9)
INSERT INTO friendbook_sizepost
VALUES('BEAUTIFUL',100,'PICTURE',10)
INSERT INTO friendbook_sizepost
...
INSERT INTO friendbook_comment
VALUES('CANNOT BE BROUGHT FROM MONEY!',480000,'VIDEO',480,12)

/* QUERY 1 */

SELECT DISTINCT A.NAME
FROM friendbook_author A,friendbook_sizepost B
WHERE A.ID=B.AUTHOR AND LEN(B.POST)<10
ORDER BY A.NAME
...
WHERE A.ID IN(  SELECT B.AUTHOR
				...
				WHERE B.ID=C.POST
				GROUP BY B.AUTHOR
				HAVING COUNT(B.POST)+COUNT(C.COMMENT)>30)
...
SELECT A.NAME,B.POST,C.COMMENT
FROM friendbook_author A,friendbook_sizepost B,friendbook_comment C
WHERE B.ID=C.POST AND A.ID=B.AUTHOR AND A.NAME LIKE 'A%' 

/* QUERY 5 */

SELECT A.CITY,A.COUNTRY
FROM friendbook_author A,friendbook_sizepost B
WHERE A.ID=B.AUTHOR AND B.POST LIKE '%HELLO%'

/* QUERY 6 */

SELECT C.COMMENT
FROM friendbook_comment C
WHERE AUTHOR IN (SELECT A.ID
				 FROM friendbook_author A
				 WHERE A.AGE<=30) 
	  AND POST IN (SELECT B.ID
			       FROM friendbook_author A,friendbook_sizepost B
				   WHERE A.ID=B.AUTHOR AND A.NAME='LEONARDO')
...
WHERE AUTHOR IN (SELECT A.ID
	  ...
	  AND POST IN (SELECT B.ID
			       FROM friendbook_author A,friendbook_sizepost B
				   WHERE A.ID=B.AUTHOR AND A.CITY='DURGAPUR' AND A.COUNTRY='INDIA')

/* QUERY 8 */

ALTER TABLE friendbook_author
ADD ADDRES VARCHAR(20)

UPDATE friendbook_author
SET ADDRES='HOME'
WHERE AGE< 30
...
GROUP BY B.AUTHOR
HAVING COUNT(B.POST)=2

SELECT *
FROM   friendbook_sizepost B
WHERE  ROW_NUMBER(SELECT *
		FROM friendbook_sizepost B
		GROUP BY B.AUTHOR,B.POST
		HAVING COUNT(B.POST)=2)=2

/*
SELECT A.NAME
FROM friendbook_author A
WHERE A.AGE BETWEEN 45 AND 48   
*/
